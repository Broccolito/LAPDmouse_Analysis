...
m10_threshold = median(filter(d,id=="m10")$depo)+qnorm(0.99)*sd(filter(d,id=="m10")$depo)
...
d_m10 = filter(d,id=="m10") %>%
  mutate(hotspot_color = ifelse(depo>m10_threshold,"black","gray60")) %>%
  mutate(hotspot_shape = ifelse(depo>m10_threshold,15,16))
...
plt2 = ggplot(data = d, aes(x = distance,y = depo)) + 
  ...
  theme(text = element_text(size = 20),
        ...
        axis.text = element_text(colour = "black"));plt2
...
# 1 micron
summary(lm(depo~distance,data = d_m10))$coefficient
# 2 micron
