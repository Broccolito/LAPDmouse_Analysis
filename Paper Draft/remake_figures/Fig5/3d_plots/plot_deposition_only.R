...
plot_depo = function(airway_data, acini_depo, filename = "m08", plotting = TRUE){
  ...
  # hotspots = acini_depo[acini_depo$mean >= depo_median + qnorm(0.95)*depo_sd,]
  ...
  if(plotting){
    ...
    for(j in 1:dim(hotspots)[1]){
      ...
      shade3d(translate3d(scale3d(cube3d(col = "coral"),
...
plot_depo(airway_data_m08, acini_depo_m08, "m08_depo_only", T) # 1 micron
plot_depo(airway_data_m27, acini_depo_m27, "m27_depo_only", T) # 2 micron
plot_depo(airway_data_m34, acini_depo_m34, "m34_depo_only", T) # 0.5 micron
